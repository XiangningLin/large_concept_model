#!/bin/bash
# LCM Preprocess OnStart Wrapper (VastAI CLI compatible - under 4048 chars)
...
echo "======================================"
echo "[$(date)] LCM Preprocess Startup Wrapper"
echo "======================================"
...
SCRIPT_PATH="/tmp/lcm_preprocess_full.sh"

download_script() {
    local ref="$1"
    local url="https://api.github.com/repos/XiangningLin/large_concept_model/contents/vastai_onstart_scripts/lcm_preprocess_onstart.sh?ref=${ref}"
...
echo "[$(date)] Downloading full preprocess script via GitHub API..."
...
if [ $EXIT_CODE -eq 0 ]; then
    echo "======================================"
    echo "[$(date)] ✅ Preprocessing completed successfully!"
    echo "======================================"
else
    echo "======================================"
    echo "[$(date)] ❌ Preprocessing failed with exit code $EXIT_CODE"
    echo "======================================"
fi
